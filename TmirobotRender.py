...
class TmirobotRender:
    def __init__( self, PTS_BASE ):
        print tcolor.OKGREEN, '--------------TmirobotRender::__init__--------------', tcolor.ENDC
        ...
        if len(self.folder_list) == 0:
            print tcolor.FAIL, 'Doesnot look like tmirobot image data', tcolor.ENDC
            quit()
        print tcolor.OKGREEN, 'END--------------TmirobotRender::__init__--------------', tcolor.ENDC
    
    def _tuple_to_filename( self, t ):
        filename = '%s/%03d/%03d%03d_pitch1_yaw%d.jpg' %(self.PTS_BASE, t[0], t[0], t[1], t[2])
        return filename

    def _query( self, exclude=[-1,-1,-1]):
        """ Generate a 3-tuple. (folderId, imageId, yawId)
                folderId : 000, 001, ..., 005
                imageId  : 000, 001, ..., 099
                yawid    : 1, 2, ..., 12

                exclude is also a 3-tuple
        """
        assert len(exclude)==3, "Length of exclude is not 3"

        if self.folder_list is None:
            folderId = np.random.randint( 5 )
            while folderId == exclude[0]:
                folderId = np.random.randint( 5 )
        else:
            folderId = random.choice( self.folder_list )

        imageId  = np.random.randint( 100 )
        while imageId == exclude[1]:
            imageId  = np.random.randint( 100 )

        yawId    = np.random.randint( 12 ) + 1
        while yawId == exclude[2]:
            yawId    = np.random.randint( 12 ) + 1

        return [folderId, imageId, yawId]
    ...
    def _similar_to( self, n, t ):
        ...
        #only change yaw, ie. t[2]
        new_yaw = np.random.randn(n)*1. + t[2]
        ...
        for i in range(1, n):
            ...
            out[2] = int(np.floor(new_yaw[i])) % 12 + 1
            L.append( out )
    ...
    def step( self, nP, nN, apply_distortions=False, return_gray=False, resize=None, ENABLE_IMSHOW=False ):
        ...
        if ENABLE_IMSHOW:
            ...
            xstr = 'folderID=%d;imageID=%d;pitchID=%d;yawID=%d' %(q_tup[0], q_tup[1], 1, q_tup[2] )
            ...
            for z in range( len(sim_im) ):
                if z == 0:
                    xstr = 'folderID=%d;imageID=%d;pitchID=%d;yawID=%d' %( (sim_tup[z])[0], (sim_tup[z])[1], 1, (sim_tup[z])[2] )
                else:
                    xstr = '  %d;  %d;  %d;  %d' %( (sim_tup[z])[0], (sim_tup[z])[1], 1, (sim_tup[z])[2] )
            ...
            for z in range( len(dif_im) ):
                if z == 0:
                    xstr = 'folderID=%d;imageID=%d;pitchID=%d;yawID=%d' %( (dif_tup[z])[0], (dif_tup[z])[1], 1, (dif_tup[z])[2] )
                else:
                    xstr = '  %d;  %d;  %d;  %d' %( (dif_tup[z])[0], (dif_tup[z])[1], 1, (dif_tup[z])[2] )
    ...
    def step_n_times( self, n_samples, nP, nN, return_gray=False, resize=(320,240), ENABLE_IMSHOW=False  ):
        print tcolor.OKGREEN, 'in fucntion TmirobotRender::step_n_times()', tcolor.ENDC
        D=[]
    ...
    def preload_all_images( self, folder_list ):
        ...
        # imageId  : 000, 001, ..., 049
        # yawid    : 1, 2, ..., 12
        estimated = len(folder_list)*100*12
        cc = 0
        for folderId in folder_list:#range(0,5):
            for imageId in range(0,100):
                for yawid in range( 1, 13 ):
                    filename = self._tuple_to_filename( [folderId, imageId, yawid] )
...
if __name__ == "__main__":
    PTS_BASE = '/home/tmirobot/Documents/place-recognition/TmirobotData/Tmirobot'
    pr = TmirobotRender( PTS_BASE )
    pr.preload_all_images( [2] )

    D = pr.step_n_times(n_samples = 25, nP=6, nN=6, resize=(320,240), return_gray=True, ENABLE_IMSHOW=True )
    quit()
